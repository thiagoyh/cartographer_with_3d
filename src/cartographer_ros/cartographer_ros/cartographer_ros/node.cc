...
Node::Node(
  ...
  if(node_options_.map_builder_options.use_trajectory_builder_3d())
  {
    point_cloud_map_publisher_ =
        node_handle_.advertise<sensor_msgs::PointCloud2>(
            "point_cloud_map", kLatestOnlyPublisherQueueSize);
  }
  ...
  {
    wall_timers_.push_back(node_handle_.createWallTimer(
        ::ros::WallDuration(10), &Node::PublishPointCloudMap, this));
  }
}
...
bool Node::HandleWriteState(
  ...
  if(node_options_.map_builder_options.use_trajectory_builder_3d() && node_options_.saved_pcd)
  {
    ...
    pcl::io::savePCDFileASCII(preflix + ".pcd", *pcl_point_cloud_map);
    LOG(INFO) << "Pcd written to " << preflix << ".pcd";
  }
